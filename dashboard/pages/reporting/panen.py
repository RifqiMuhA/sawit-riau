...
dash.register_page(__name__, path="/reporting/panen", name="Realisasi Panen", order=3)
...
def update_kpi(pid, periode):
    ...
    k = _kpi_panen(pid, p)
    return dbc.Row([
        dbc.Col(_kpi(f"{k['total_target']:,.0f} ton",   "Total Target",       "🎯", ""),       lg=3, md=6, sm=12, className="mb-2"),
        dbc.Col(_kpi(f"{k['total_realisasi']:,.0f} ton","Total Realisasi",    "🌾", ""),       lg=3, md=6, sm=12, className="mb-2"),
        dbc.Col(_kpi(f"{k['avg_gap'] or 0:.1f}%",       "Rata-rata Gap",      "📉", "warning"),lg=3, md=6, sm=12, className="mb-2"),
        dbc.Col(_kpi(f"{k['pct_selesai'] or 0:.1f}%",   "Panen Selesai",      "✅", ""),       lg=3, md=6, sm=12, className="mb-2"),
    ], className="g-3")
